...
@patch(
    "tap_sftp.decrypt.gpg_decrypt", return_value=get_sample_file_path("fake_file.txt")
)
def test_get_file_handle_decrypt(patch_decrypt, sftp_client):
    """
    File handle of gpg file calls the decrypt method
    """
    sftp_client.get_file_handle(
        {"filepath": "/path"},
        {"key": "key", "gnupghome": "gnupghome", "passphrase": "passphrase"},
    )
