...
def sync_stream(config, state, stream):
    ...
    if len(table_spec) == 0:
        LOGGER.info(
            "No table configuration found for '%s', skipping stream", table_name
        )
        return 0
    if len(table_spec) > 1:
        LOGGER.info(
            "Multiple table configurations found for '%s', skipping stream", table_name
        )
        return 0
    table_spec = table_spec[0]

    files = sftp_client.get_files(
        table_spec["search_prefix"], table_spec["search_pattern"], modified_since
    )
    sftp_client.close()
    ...
    for sftp_file in files:
        ...
        state = singer.write_bookmark(
            state, table_name, "modified_since", sftp_file["last_modified"].isoformat()
        )
...
def sync_file(sftp_file_spec, stream, table_spec, config):
    ...
    opts = {
        ...
        "field_names": table_spec.get("field_names"),
